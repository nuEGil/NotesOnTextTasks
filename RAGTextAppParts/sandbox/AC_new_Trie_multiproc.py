...
---- Threading is really so that you can have one thread that is dealing with file i/o
and the other thread is handling processing.... 

for this part you want multiprocessing... 
'''
def GetText(x):
...
def Job(kargs):
    ...
    data_part = []   
    for fi in range(kargs['mdata'].shape[0]):
    # read the text file 
        text = GetText(kargs['mdata']['title'].iloc[fi])
        sub_title = kargs['mdata']['title'].iloc[fi].split('/')[-1].replace('.txt','')        
        # search for the words in word set 
        word_set = kargs['Trie_'].TextSearch(text.lower())
...
if __name__ == '__main__':
    ...
    # now you can mapt the batchches to the pool -- use with instead of start and join
    with mp.Pool(n_threads) as procs:
        output = procs.map(Job, [{'mdata':b, 'Trie_':Trie_} for b in batches])
