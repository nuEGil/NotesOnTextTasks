...
def modeltraining(odir, odata, 
    ...
    for steps in range(N_steps):
        ...
        if steps%100 == 0:
            ...
            tokens_oout = model.generate(torch.zeros((1,1), dtype=torch.long, device=device), max_new_tokens=100)
...
if __name__ =='__main__':
    odir = '/mnt/f/data/models/bigram/bugcheck'
    # read text
    text_dir = "/mnt/f/data/ebooks_public_domain/"
   
    #--- Model initialization ---#
    # model training arguments
    sequence_length = 16 # max sequence length 
    ...
    model =  MyModel(odata['vocabulary_size'], 
                     odata['token_probs'], 
                     emb_dim=embed_dim, 
                     seq_len = sequence_length,
                     n_out = odata['vocabulary_size']).to(device)
    print(model)
    ...
    logg, model, optimizer = modeltraining(odir, odata, 
                    device, model, loader, optimizer,
                    N_steps = 500,
                    tag=0)

    # experiment with passing the optimizer state between runns. 
    # --- swap data set ---#
    # train model on book1... 
    print('\n\n-----Swaping data set-----')
    del text 
    del loader
    del data_set
    ...
    # there has to be a loop for this